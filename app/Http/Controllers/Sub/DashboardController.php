    ...
    public function index()
    {
        ...
        $query = "SELECT user_id,
                ...
                (SELECT users.id AS user_id,
                        ...
                        (SELECT companies.short_name FROM companies WHERE companies.comp_id = users.company_id) as company_name,
                    ...
                    WHERE tickets.supporter_uid IS NOT NULL
                    AND tickets.tkt_dprtmnt = $serv_dept_id) AS ss
                GROUP BY user_id,
                        username,
                        emp_id
                ORDER BY u_status ASC, total DESC";
        ...
        $weeklyTkts = Ticket::select(DB::raw("(COUNT(*)) as count"), DB::raw('DATE_FORMAT(created_at, "%W") as dayname'))
                        //->whereBetween('created_at', [Carbon::now()->startOfWeek()->subDays(), Carbon::now()->endOfWeek()])
        ...
        return view('backend.sub.index', ['usertTktByStatus'=> $usertTktByStatus, 'labels' => $labels, 'data' => $data, 'bg_color'=>$color, 'clabels' => $clabels, 'cdata' => $cdata, 'cbg_color'=>$ccolor,'mlabels' => $mlabels, 'mdata' => $mdata, 'wlabels' => $wlabels, 'wdata' => $wdata, 'statuses'=>$statuses, 'pending' => $pending,'cmplabels'=>$cmplabels, 'cmpdata'=>$cmpdata,'tcmplabels'=>$tcmplabels, 'tcmpdata'=>$tcmpdata, 'today_total'=>$tcmptkts,'today_total'=>$today_total]);
    }

    public function show(Request $request)
    {
        return view('backend.sub.profile.index');
    }
